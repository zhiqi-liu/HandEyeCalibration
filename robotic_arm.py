...
def ur3e_model():
    ...
    L6 = RevoluteDH(
        d=0.0921, a=0, alpha=0, offset=0, qlim=[-2 * np.pi, 2 * np.pi], m=0.261, r=[0, 0, -0.02], I=[0, 0, 0.0001]
    )
    robot = DHRobot([L1, L2, L3, L4, L5, L6], name="ur3e")
    return robot


def rm65_6FB_model(gripper=False):
    """
    构建 RM65 机械臂的 DH 参数模型。
    本函数定义了 RM65 机械臂的每个关节的 DH 参数,
    并返回一个基于 `roboticstoolbox` 库的 `DHRobot` 对象。
    每个关节均为旋转关节(RevoluteMDH)。
        Returns
    -------
    robot : DHRobot
        一个包含 6 个旋转关节的 RM65 机械臂模型对象。
    Notes
    -----
    - 本模型基于 RM65 的典型几何尺寸(以米为单位)。
    - 质量 `m`、质心位置 `r`、惯量 `I` 仅为近似值, 用于动力学或仿真计算。
    - 每个关节的关节角限制(`qlim`)根据 RM65 的规格设置。
    - 如果 `gripper` 参数为 True, 则在机械臂末端添加一个夹爪工具, 其长度为 0.2038 米。
    DH 参数说明
    ------------
    - `d` : 连杆偏移量(沿 z_{i-1})
    - `a` : 连杆长度(沿 x_i)
    ...
    DH_ = [
        ...
        [0, 90, 161.2, 0, -360, 360, 225, 0.248, -0.426, 0.237, -27.223, 308.844, -3.781, -1.468, 304.616, 0.888, 122.62],
    ]
    # fmt:on
    L1 = RevoluteMDH(
        a=1e-3 * DH_[0][0],
